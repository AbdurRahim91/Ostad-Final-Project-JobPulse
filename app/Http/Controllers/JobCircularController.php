...
class JobCircularController extends Controller
{
    ...
    public function show_alljobs_by_category(string $id)
    {
        ...
        return view('frontend.pages.jobs_listcategory', compact('jobsList', 'cat_id'));
        //dd($cat_id);
    }
    ...
    {
        // dd('jobs');
        $jobPage = JobCircular::latest()->paginate(10); // Change 10 to the desired number of items per page
        return view('frontend.pages.jobs', compact('jobPage'));
    }
  ...
  $job_details = DB::table('job_applications')
      ...
      //dd("$job_details");
       //$existingJob = JobCircular::findOrFail($id);
        if ($job_details) {
            return view('dashboard.pages.user.jobs_application', compact('job_details'));
        } else {
    ...
    public function jobList()
    {
        ...
        ->get();
        //dd($jobsLists);
        return view('dashboard.pages.company.jobList', compact('jobsLists'));
    }
    ...
    public function applicants_list(Request $request)
    {
        ...
        //dd( $applicantsLists);
        return view('dashboard.pages.company.apllicants_list', compact('applicantsLists'));
    }
    ...
    public function applicant_details(string $id)
    {
        ...
        ->first();
        //dd($applicantsinfo);
        return view('dashboard.pages.company.applicant_details', compact('applicantsinfo'));
    }
    ...
    public function applicants_update(Request $request)
    {
        ...
        if($job_apply){
            ...
            if($result){
                return redirect()->route('company/applicant_details', $id)->with('success', 'Post updated successfully');
            } else {
                return redirect()->route('company/applicant_details', $id)->with('fail', 'Post Not updated');
            }
        } else {
    ...
    public function show_jobs_for_id(string $id)
    {
        ...
        return view('dashboard.pages.company.show',compact('jobList'));
    }
    /**
    ...
    {
        $jobList = JobCircular::findOrFail($id);
        return view('dashboard.pages.company.edit', compact('jobList'));
    }
...
$result=$jobList->update([
    ...
    public function destroy(string $id)
    {
        $jobList = JobCircular::findOrFail($id);
        $jobList->delete();
        return redirect('job-list')->back()->with('success', 'Post deleted successfully');
    }
}
